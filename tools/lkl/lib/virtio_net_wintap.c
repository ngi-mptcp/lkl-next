...
/* Format lasterror */
static wchar_t *GetErrorMessage()
{
...
void show_tap_win_adapters (void)
{
	...
	if (warn_panel_dup)
		tap_debug_printf("WARNING: Some TAP-Windows adapters have duplicate "
			   "links from the Network Connections control panel\n");

	if (warn_panel_null)
		tap_debug_printf("WARNING: Some TAP-Windows adapters have no link "
			   "from the Network Connections control panel\n");
}
...
static int open_tun(const char *dev_node, struct lkl_netdev_wintap *nd)
{
	...
	tap_debug_printf("open_tun, nd->ipv6=%d\n", nd->tt->ipv6);
	/*
	...
	/* get driver version info */
	{
		...
		if (!(info[0] == TAP_WIN_MIN_MAJOR && info[1] >= TAP_WIN_MIN_MINOR))
			lkl_printf("ERROR:  This requires a TAP-Windows driver that is at least version %d.%d.\n",
				      TAP_WIN_MIN_MAJOR,
				      TAP_WIN_MIN_MINOR);
		 ...
		 */
		if (nd->tt->ipv6 && nd->tt->type == DEV_TYPE_TUN &&
		    info[0] == 9 && info[1] < 8) {
			lkl_printf("WARNING:  Tap-Win32 driver version %d.%d \
			does not support IPv6 in TUN mode.  IPv6 will be disabled.\
			Upgrade to Tap-Win32 9.8 (2.2-beta3 release or later) \
			or use TAP mode to get IPv6\n",
		 ...
		 */
		if (nd->tt->type == DEV_TYPE_TUN && info[0] == 9 && info[1] == 8) {
			lkl_printf("ERROR:  Tap-Win32 driver version %d.%d is \
					buggy regarding small IPv4 packets in\
					TUN mode.  Upgrade to Tap-Win32 9.9 \
					(2.2.2 release or later) or use TAP mode\n",
				   (int) info[0], (int) info[1] );
		}
	}
	...
	/* set driver media status to 'connected' */
	{
		...
		if (!DeviceIoControl(nd->tt->hand, TAP_WIN_IOCTL_SET_MEDIA_STATUS,
			...
			lkl_printf("WARNING: The TAP-Windows driver rejected a TAP_WIN_IOCTL_SET_MEDIA_STATUS DeviceIoControl call.");
	}
	...
	if (tt->reads.iostate == IOSTATE_INITIAL) {
		...
		if (status) { /* operation completed immediately? */
			...
			if (err == ERROR_IO_PENDING) { /* operation queued? */
				...
				tt->reads.iostate = IOSTATE_IMMEDIATE_RETURN;
				tt->reads.status = err;
				lkl_printf("WIN32 I/O: TAP Read error [%d] : %d\n", (int) len, (int) err);
			}
		}
	}
...
int tun_finalize(HANDLE h, struct overlapped_io *io, uint8_t **buf, uint32_t *buf_len)
{
	...
	case IOSTATE_IMMEDIATE_RETURN:
		...
		if (io->status) {
			...
			ret = io->size;
			tap_debug_printf("WIN32 I/O: TAP Completion non-queued success [%d]\n", ret);
		}
...
static int wintap_rx(struct lkl_netdev *nd, struct iovec *iov, int cnt)
{
	...
	for (i = 0; i < cnt; i++) {
		int ret = read_tun_buffered(nd_wintap->tt, recv_buffer, 1500);
		if (ret > 0) {
...
static int wintap_tx(struct lkl_netdev *nd, struct iovec *iov, int cnt)
{
	int i, ret, err, bytes = 0;
	struct lkl_netdev_wintap *nd_wintap =
		container_of(nd, struct lkl_netdev_wintap, dev);

	tap_debug_printf("%s: write\n", __func__);
	if (!nd_wintap->writes.overlapped.hEvent)
		nd_wintap->writes.overlapped.hEvent =
			OpenEvent(EVENT_ALL_ACCESS, FALSE, "write");
	...
	for (i = 0; i < cnt; i++) {
		...
		memcpy(dst, iov[i].iov_base, 6);
		memcpy(src, iov[i].iov_base + 6, 6);
		tap_debug_printf("%s: tx dst=%02x:%02x:%02x:%02x:%02x:%02x"
			   ", src=%02x:%02x:%02x:%02x:%02x:%02x\n", __func__,
			   dst[0],
			   dst[1],
			   dst[2],
		...
		if (nd_wintap->reads.iostate == IOSTATE_QUEUED) {
			ret = overlapped_io_process(nd_wintap->handle,
						    &(nd_wintap->writes),
						    iov[i].iov_base, __func__);
			if (ret < 0)
				return -1;
			continue;
		}

		if (!ResetEvent(nd_wintap->writes.overlapped.hEvent))
			lkl_printf("%s: ResetEvent failed, %s",
				   __func__, GetErrorMessage());

		ret = WriteFile(nd_wintap->handle, iov[i].iov_base,
				iov[i].iov_len,
				&nd_wintap->writes.size,
				&nd_wintap->writes.overlapped);

		if (ret) {
			nd_wintap->writes.iostate = IOSTATE_IMMEDIATE_RETURN;

			if (!SetEvent(nd_wintap->writes.overlapped.hEvent))
				lkl_printf("%s: SetEvent in write_queue failed!\n",
					   __func__);

			nd_wintap->writes.status = 0;

			tap_debug_printf("WIN32 I/O: TAP Write immediate return [%d,%d]\n",
				       (int)(nd_wintap->writes.buf_len),
				       (int)nd_wintap->writes.size);
		}
		else {
			err = GetLastError ();
			if (err == ERROR_IO_PENDING) {
				nd_wintap->writes.iostate = IOSTATE_QUEUED;
				nd_wintap->writes.status = err;
				tap_debug_printf("WIN32 I/O: TAP Write queued [%d]\n",
					       (nd_wintap->writes.buf_len));

			}
			else {
				lkl_printf("%s: I/O error: %d\n", __func__,
					   GetLastError());
				nd_wintap->reads.iostate = IOSTATE_IMMEDIATE_RETURN;
				return -1;
			}
		}

		iov[i].iov_len = nd_wintap->writes.size;
		bytes += iov[i].iov_len;
	}

	tap_debug_printf("wintap_tx: written size=%d\n", bytes);
	return bytes;
}

static int wintap_rx(struct lkl_netdev *nd, struct iovec *iov, int cnt)
{
	int i, ret = 0, err, bytes = 0;
	struct lkl_netdev_wintap *nd_wintap =
		container_of(nd, struct lkl_netdev_wintap, dev);

	tap_debug_printf("wintap_rx: read\n");
	for (i = 0; i < cnt; i++) {
		if (nd_wintap->reads.iostate == IOSTATE_QUEUED) {
			ret = overlapped_io_process(nd_wintap->handle,
						    &(nd_wintap->reads),
			...
			nd_wintap->reads.buf = nd_wintap->reads.buf_init;
			nd_wintap->reads.buf_len = nd_wintap->reads.buf_init_len;
			memset(&nd_wintap->reads.buf, 0, nd_wintap->reads.buf_len);

			if (!ResetEvent(nd_wintap->reads.overlapped.hEvent))
				lkl_printf("%s: ResetEvent failed, %s",
					   __func__, GetErrorMessage());

			ret = ReadFile(nd_wintap->handle,
				       &nd_wintap->reads.buf,
				       iov[i].iov_len,
				       &nd_wintap->reads.size,
				       &nd_wintap->reads.overlapped);
		}
		if (ret) {
			/* We got immediate return */
			nd_wintap->reads.iostate = IOSTATE_IMMEDIATE_RETURN;
			if (!SetEvent(nd_wintap->reads.overlapped.hEvent))
				lkl_printf("%s: SetEvent failed\n", __func__);
		}
		else {
			err = GetLastError ();
			if (err == ERROR_IO_PENDING) {
				nd_wintap->reads.iostate = IOSTATE_QUEUED;
				nd_wintap->reads.status = err;
				tap_debug_printf("%s: I/O pending\n", __func__);
			}
			else {
				lkl_printf("%s: I/O error: %s\n", __func__,
					   GetErrorMessage());
				if (!SetEvent(nd_wintap->reads.overlapped.hEvent))
					lkl_printf("%s: SetEvent failed\n", __func__);
		...
		iov[i].iov_len = nd_wintap->reads.size;
		bytes += iov[i].iov_len;
	}

	tap_debug_printf("%s: state=%d, ret=%d, read=%d\n", __func__,
		   nd_wintap->reads.iostate, ret, bytes);
	if (bytes > 0) {
		uint8_t src[6], dst[6];
		memcpy(dst, iov->iov_base, 6);
		memcpy(src, iov->iov_base + 6, 6);
		tap_debug_printf("%s: rx dst=%02x:%02x:%02x:%02x:%02x:%02x"
			   ", src=%02x:%02x:%02x:%02x:%02x:%02x\n", __func__,
			   dst[0],
			   dst[1],
...
//		tap_debug_printf("%s: write ready\n", __func__);
